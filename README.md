...
以下は、GROWTH PARTNERを実際の業務で利用する場面を想定した代表的なユースケースです。
...
**利用シーン**  
業務開始時に当日の作業を確認したところ、「保険料計算ロジックの実装」タスクが期限を超過しており、同じ日に単体テストと定例会議も予定されている状況を想定します。

**操作の流れ**
1. 利用者は「今日」画面を開き、本日期限のタスクと期限超過タスクを確認します。
2. 期限超過のタスクを選択し、タスク詳細からメモ、優先度、サブタスクを確認します。
3. 「AIタスク自動調整」を実行し、他の予定や未完了タスクを考慮した新しい期日案を確認します。
4. 必要に応じてカレンダー上で作業時間を再配置し、提案された期日を調整します。
5. 納期変更についてPMへの相談が必要な場合は、AIが作成した相談メールの下書きを確認・編集し、本人の判断でGmailの下書き画面を開きます。
6. 作業完了後、タスクを「完了」に変更します。ダッシュボードの完了率とタスク件数にも変更内容が反映されます。

**期待される成果**
- 業務開始時に、着手すべき作業と遅延リスクを短時間で把握できます。
- タスクの再調整からPMへの相談文作成までを一連の流れで進められます。
- AIの提案をそのまま確定せず、利用者が内容を確認して最終判断できます。
...
**利用シーン**  
退勤前に、当日のGitコミットログや作業メモを基に、上司へ提出する日報を作成する状況を想定します。

**入力例**
```text
- 保険料計算APIの入力チェックを追加
- 契約更新処理の単体テストを3件作成
- 解約返戻金の計算仕様についてオフショアチームへ確認
- 保険料計算ロジックの境界値テストが未完了。明日午前中に対応予定
```

**操作の流れ**
1. 利用者は左メニューの「日報作成」を開きます。
2. Gitのコミットログや当日の作業メモを入力欄へ貼り付けます。
3. AIが入力内容を整理し、「本日の実施内容」「進捗」「特記事項」の構成で日報の下書きを生成します。
4. 利用者は、実際の進捗、未完了事項、翌日の対応予定が正しく記載されているか確認し、必要に応じて本文を修正します。
5. 「上司へ報告書を送信（Gmail）」を選択し、Gmailの下書き画面で宛先と最終内容を確認します。アプリからメールが自動送信されることはありません。

**生成結果の例**
```text
【本日の実施内容】
・保険料計算APIの入力チェックを追加しました。
・契約更新処理の単体テストを3件作成しました。
・解約返戻金の計算仕様について、オフショアチームへ確認しました。

【進捗】
保険料計算ロジックの実装および関連するテストを進めています。
境界値テストは未完了のため、明日午前中に対応する予定です。

【特記事項】
解約返戻金の計算仕様について確認中です。回答を受領後、実装への影響を確認します。
```

**期待される成果**
- 断片的な作業ログから、上司が状況を把握しやすい日報を短時間で作成できます。
- 未完了事項と次のアクションを明確にし、翌日の作業へつなげられます。
- 送信前に本人が編集・確認することで、誤った内容の報告を防止できます。
...
![画面構成（今日のタスク）](image/todaytask.png)

---

## 6. 主な機能

### 6.1 今日のタスク（デイリーマネジメント）
...
![今日のタスク](image/todaytask.png)

---

### 6.2 プロジェクトダッシュボード（進捗の可視化）
...
![AIタスク自動調整](image/AIchatbot.png)

---

### 6.7 緊急SOS（行き詰まりの早期共有）
