...
let demoAiNoticeShown = false;

// APIキーが未設定でも、ハッカソンの導線を止めないためのローカルデモ応答。
// APIキーを設定すると、この分岐は使われず通常のGemini APIへ接続する。
function getDemoAiResponse(url, body = {}) {
  if (!demoAiNoticeShown) {
    demoAiNoticeShown = true;
    showToast("✨ デモモード", "APIキー未設定のため、サンプル結果で処理を続けています。", "info");
  }

  if (url.includes("analyze-schedule")) {
    const rebalancedTasks = (body.tasks || []).slice(0, 3).map((task, index) => ({
      taskId: task.id,
      title: task.title,
      delayDays: index + 2,
      newDueDate: toDateKey(addDays(new Date(task.dueDate), index + 2)),
    }));
    return Promise.resolve({
      rebalancedTasks,
      findingsSummary: "【デモ分析】期限超過タスクを優先度と負荷のバランスに基づいて再配置しました。まずは最優先タスクから着手し、PMへ新しい期日案を共有してください。",
      draftEmailBody: "お疲れさまです。遅延しているタスクについて、作業順序と空き時間を見直した新しい期日案を作成しました。内容をご確認いただき、必要であればご相談させてください。",
    });
  }

  if (url.includes("sos-alert")) {
    return Promise.resolve({
      alertMessage: "【デモ分析】PremiumCalculator.java の実装が180分停滞しています。仕様確認またはレビュー支援が必要な可能性があります。",
      slackMessageDraft: "お疲れさまです。PremiumCalculator.java の実装で停滞しており、現在180分ほど進捗が止まっています。仕様の確認と実装方針についてご相談させていただけますでしょうか。",
    });
  }

  if (url.includes("generate-nippo")) {
    return Promise.resolve({
      nippoText: `【本日の実施内容】\n${body.rawLogs || "タスクの確認と実装作業を行いました。"}\n\n【進捗】\n主要タスクの状況を確認し、次のアクションを整理しました。\n\n【特記事項】\n特にございません。`,
    });
  }

  if (url.includes("review-offshore")) {
    const mode = body.mode || "";
    const analysisText = mode === "SPEC_DIFF"
      ? "【デモ分析】仕様では18歳以上が成人ですが、コードは20歳以上を判定しています。境界値18・19歳のテストを追加し、仕様または実装のどちらを正とするか確認してください。"
      : mode === "SHADOW_CLIENT"
        ? "【デモ分析】質問内容を確認しました。前提条件を整理し、対象範囲と確認したい仕様を明確にすると、より丁寧で回答しやすい質問になります。"
        : "【デモ分析】正常系・境界値・入力不備・二重実行の観点を含むテストケースのたたき台を作成しました。実装方針に合わせて最終確認してください。";
    return Promise.resolve({
      analysisText,
      riskWarningText: "これはデモ結果です。実案件へ適用する前に、仕様とレビュー担当者の判断で内容を確認してください。",
    });
  }
...
async function requestJson(url, method, body) {
  if (isAiEndpoint(url) && !getStoredUserApiKey()) {
...
// CSVは実データを読み込み、Excel/PDF/TXTはハッカソン用のフォールバックデータで確認できる。
async function handleWbsFileImport(inputEl) {
  ...
  let importedTasks = fallbackTasks;
  if (/\.csv$/i.test(file.name)) {
    try {
      const csvTasks = parseWbsCsv(await file.text(), targetProjectId);
      if (csvTasks.length) importedTasks = csvTasks;
      else {
        showToast("⚠️ CSVを確認してください", "タスク名の列が見つからなかったため、取り込みを中止しました。", "warning");
        inputEl.value = "";
        return;
      }
    } catch (error) {
      showToast("⚠️ CSVを読み込めませんでした", "ファイル形式を確認してください。", "warning");
      inputEl.value = "";
      return;
    }
  }
  ...
  showToast("✅ インポート完了", `${isCsv ? "CSVから" : "デモデータとして"}${importedTasks.length}件のタスクを追加しました。`, "success");
  inputEl.value = "";
}
